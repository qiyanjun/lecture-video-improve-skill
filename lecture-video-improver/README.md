...
└── scripts/                              ← every script below is called by its number
    ...
    ├── retry_thumbnails.py               ← Stage 2: standalone thumbnail-rate-limit retry pass
...
- **Real bug found and fixed: `upload_to_youtube.py`'s playlist-add step wasn't wrapped in non-fatal error handling**, unlike the thumbnail-set step right below it. A failure there (e.g. `quotaExceeded` hitting mid-batch, after `videos.insert()` already succeeded) propagated uncaught, crashing the whole upload process and causing the caller to mark an already-live video "failed" -- orphaning it (untracked in `publish_log.json`, absent from the playlist) and inviting a duplicate re-upload on the next retry, since the job manifest still showed it as not-yet-published. This produced one confirmed real incident (an orphaned duplicate of L22-M2), found via a channel-video-count mismatch, root-caused via direct API inspection, and cleaned up manually. Fixed by wrapping the playlist-add call in the same try/except pattern already used for thumbnails: on failure, print a warning and return the video ID anyway rather than raising.

### Why the 0.17.8 changes exist

- **Serious real bug found and fixed: `run_stage2_publish.py` recorded the wrong video ID for every job uploaded after the source-recording citation feature shipped (0.17.2+).** The log-recording regex, `re.search(r"watch\?v=([\w-]+)", output)`, matches the FIRST `watch?v=...` URL anywhere in a job's combined output -- and once descriptions started citing the original source recording (its own `youtube.com/watch?v=...` URL, printed as part of the "Plan:" preview *before* the real upload even runs), that citation's video ID won over the actual `Uploaded: https://youtube.com/watch?v=...` confirmation line further down. `publish_log.json` ended up recording the SOURCE recording's ID as if it were the newly-uploaded video's own ID. Confirmed on a real 13-video batch: every single one wrong, discovered when `retry_thumbnails.py` tried to set thumbnails using these IDs and got `403 forbidden -- not properly authorized` (correctly, since we don't own the original 2020 recordings). Fixed by anchoring the regex to the literal `"Uploaded: "` prefix so it can only match the real confirmation line. Verified against the actual failing log: all 13 previously-wrong extractions now match the correct ID. The earlier 58 videos (uploaded before the citation feature existed, so no `watch?v=` collision was possible) were independently confirmed unaffected. Manually corrected the 13 wrong entries in `publish_log.json` and re-ran the thumbnail fix with the right IDs.

### Why the 0.17.7 changes exist

- **Reordered description sections**: the course syllabus link now comes immediately after the original-recording link (nothing in between), with the course description blurb moved after that. Previously the blurb sat between the two links.

### Why the 0.17.6 changes exist

- **Real bug found and fixed: neither `run_stage2_publish.py` nor `retry_thumbnails.py` self-stopped on the general API `quotaExceeded` error** -- only on their respective account-standing/rate-limit errors (`uploadLimitExceeded`, `uploadRateLimitExceeded`). Confirmed for real: once the shared 10,000-unit/day quota was exhausted (from several retroactive description-update passes in one day), a `retry_thumbnails.py` run kept attempting -- and failing -- every remaining thumbnail instead of stopping after the first `quotaExceeded`, the exact failure mode the earlier self-stop fixes were written to prevent, just for a different error. Both scripts now treat `quotaExceeded` as an equally valid stop condition alongside their existing checks.

### Why the 0.17.5 changes exist

- **Real bug found and fixed: the double-slash fix in 0.17.4 wasn't actually the cause.** After fixing that and retroactively updating every video, the course-syllabus link was STILL not clickable on the real YouTube watch page, while the original-source-recording link (also auto-generated, right above it) worked fine. The actual difference: the working link puts its label and URL on separate lines; the broken one had `"Course syllabus: {url}"` on one line, colon-space directly before the URL. Changed to match the confirmed-working pattern -- label and URL each on their own line. (The double-slash normalization from 0.17.4 is still correct and worth keeping, just wasn't sufficient on its own.)
- **Also surfaced a third, distinct rate limit while retroactively fixing this**: the general YouTube Data API quota (10,000 units/day, shared pool) -- separate from both the daily video-upload cap and the thumbnail-set rate limit this plugin already handles. Several retroactive description-update passes in one day (each `videos.update` costs ~50 units) exhausted it. Unlike the other two undocumented limits, this one resets at a known time -- midnight Pacific.

### Why the 0.17.4 changes exist

- **Real bug found and fixed: a stray double slash in a URL made YouTube's description auto-linkifier fail to render it as a clickable link**, even though the URL itself resolved fine (HTTP 200) either way -- a working URL and a URL YouTube will actually linkify turned out not to be the same thing. Added `normalize_url()` (collapses redundant consecutive slashes in the path, leaves the `://` after the scheme alone) and applied it to both the course-syllabus link and the original-source-recording link in `generate_description()`. Retroactively fixed on all previously-published videos.

### Why the 0.17.3 changes exist

- **`--course-description` adds an optional short course blurb** alongside `--course-url` (now labeled "Course syllabus:" rather than "Course website:"). Deliberately requires the caller to supply the text -- `generate_description()` never scrapes or paraphrases a course page's own content automatically; the blurb has to be explicitly provided (written by a human, or drafted by an agent and shown to the user for approval before use).

### Why the 0.17.2 changes exist

- **Generated descriptions now cite the original source recording and can include a course link.** `generate_description()` adds an "Original lecture recording: {title}" line with a link back to the job's own `source_video_url`/`_raw_title` (already stored per-job in the manifest, no new lookups needed) -- attribution only, not reproducing the original video's own description or content. `--course-url` adds an optional "Course website:" line to every description. Deliberately minimal: title + link, the same as a normal citation, not a summary of the original video's content.

### Why the 0.17.1 changes exist

- **Every upload now explicitly declares `selfDeclaredMadeForKids`** (default `false`). YouTube requires every video to declare a "made for kids" status one way or the other -- it's not optional, and getting it wrong has real consequences (`true` disables comments, notifications, and personalized ads, which is wrong for course/lecture content, this plugin's typical use case). Previously `upload_to_youtube.py` never set this field at all, leaving it to whatever default applied. `--made-for-kids` opts in for content genuinely directed at children. Defaulting to `false` (not `true`) is the safer choice for a general-purpose tool: silently defaulting to "made for kids" strips features from content that shouldn't have them stripped, while the reverse mistake is far less likely to matter in practice.

### Why the 0.17.0 changes exist

- **New: `retry_thumbnails.py`**, a standalone companion for YouTube's custom-thumbnail rate limit. Confirmed directly (and via independent sources) that `uploadRateLimitExceeded` (HTTP 429 on `thumbnails.set`) is an undocumented, rolling ~24h-per-channel window -- separate from, and much slower to clear than, the daily video-upload cap `run_stage2_publish.py` already handles. On a real batch: it tripped after only a handful of thumbnails set back-to-back during video uploads, and was still blocked a full ~24h later even though the video-upload cap itself had already reset hours earlier. A large batch essentially always needs thumbnails set in a separate pass like this one. Same self-stopping principle as the daily-cap handling: stops on the first rate-limit hit instead of attempting every remaining thumbnail.
- **Real bug found and fixed: `publish_log.json` (and the new `thumbnail_log.json`) now write after every job, not just once at the end.** Previously the log was only written once, after the entire batch loop finished. A killed or crashed process -- including, ironically, hitting the very `uploadLimitExceeded` condition this script is supposed to track -- lost the log for the WHOLE run, even for jobs that had genuinely succeeded and were already live on YouTube. Caught this for real: two runs on an actual 71-video batch had to be killed by hand before the daily-cap self-stop existed, and both lost all their successful uploads from `publish_log.json` -- the videos were live, just untracked, until manually reconstructed from raw output logs. Fixed by moving the write inside the loop.
