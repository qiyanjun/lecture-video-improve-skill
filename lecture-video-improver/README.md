...
| `upload_to_youtube.py` | 2 | **New in 0.7.0.** Uploads one finished video to YouTube with "L**" lecture-numbering title formatting (`L1: Topic` or `L1: Topic - Module 2`). Hard-fails unless `--confirmed-title` matches the computed title exactly, so an upload can't happen without the literal title having been shown to and confirmed by the user. Defaults to `private` visibility. | OAuth2 (Google Cloud project + one-time browser consent) |
| `run_stage2_publish.py` | 2 | **New in 0.8.0.** Batch version of `upload_to_youtube.py`'s confirmation gate: one dry-run computes every video's title across the whole batch into `publish_plan.json`, one user confirmation covers all of them, then `--execute --confirmed-plan` re-verifies the plan is still byte-identical (via content hash) before uploading anything. | same as `upload_to_youtube.py` |
